...
ターミナルから CLI で読んで返信する。閉じるのは指摘した人。
...
- **見た目を選ぶ** — 構文の配色をライト / ダークそれぞれに持ち、GitHub・One・
  Solarized・Nord・Dracula など 15 種から選ぶ。書体も等幅・ゴシック・明朝から
  選べる（`…` メニュー →「表示」）
- **行に指摘を書く** — 行番号をクリック（Shift で範囲）してコメントする。
  人と AI のどちらの発言かは顔で見分ける。ファイルには未解決の件数が付き、
  上部バーの吹き出しから一覧を開いて対象の行へ飛べる
- **AI と往復する** — 書いた指摘は `kodama review list` から読め、AI が
  `reply` で返す。GUI は監視していて即座に反映する。閉じるのは指摘した人
...
一言で、`kodama review list` から `reply` までの流れを踏んでくれる。
閉じる操作は含めていない。直ったかを決めるのは指摘した人。
...
**GUI は台帳を監視しているので、返信はその場で画面に反映される。**

### 4. 人間が閉じる

直ったかを決めるのは指摘した人。返信を読んで納得したら GUI で解決にする。
直した側が閉じると、その確認が飛ばされる。`resolve` と `drop` は人間の操作で、
AI 向けのスキルでは使わせない。
...
事実を状態として出し、`resolve`（対応した）か `drop`（取り下げる）を人間に
委ねる。直した側が閉じると、指摘した人の確認が飛ばされる。
